...
void TreeBillboardsApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	// VK_RIGHT 右箭头键 0x27
	// VK_DOWN 下箭头键 0x28
	if (GetKeyState(0x31) & 0x8000)  // 0x31 对应于键盘的1
	{
		mMainPassCB.TessNum = 0.0f;
	}
	if (GetKeyState(0x32) & 0x8000)
	{
		mMainPassCB.TessNum = 1.0f;
	}
	if (GetKeyState(0x33) & 0x8000)
	{
		mMainPassCB.TessNum = 2.0f;
	}
}
...
void TreeBillboardsApp::BuildShadersAndInputLayouts()
{
	...
	mShaders["opaqueGS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "GS", "gs_5_0");
...
void TreeBillboardsApp::BuildBoxGeometry()
{
	...
	BulidShphereGeometry();
}

// 创建球体几何体
void TreeBillboardsApp::BulidShphereGeometry()
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData sphere = geoGen.CreateGeosphere(1.0f, 3);
	...
	vector<uint16_t> indices = sphere.GetIndices16();
	...
	mGeometries["sphere"] = std::move(geo);
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	//
	// PSO for tessellation objects   曲面细分的pso， 基本上改成线框模式就行
	//
	auto tessPso = opaquePsoDesc;
	//tessPso.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
	tessPso.RasterizerState.FillMode = D3D12_FILL_MODE_WIREFRAME;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&tessPso, IID_PPV_ARGS(&mPSOs["tessellation"])));
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&sphereRitem->World, XMMatrixMultiply(XMMatrixScaling(5,5,5), XMMatrixTranslation(0.0f, 2.0f, -9.0f)));
	sphereRitem->ObjCBIndex = 4;
	sphereRitem->Mat = mMaterials["water"].get();
