...
void ShadowMapApp::UpdateShadowTransform(const GameTimer& gt)
{
    ...
    XMVECTOR lightPos = -0.5f*mSceneBounds.Radius*lightDir;
    ...
    XMMATRIX S = lightView*lightProj*T;  // S是将世界空间中的点变换到阴影贴图纹理空间的矩阵
...
void ShadowMapApp::BuildPSOs()
{
    ...
    smapPsoDesc.RasterizerState.DepthBias = 100000.0f;
    ...
    // 阴影贴图的渲染过程不需要渲染目标, 这里要将渲染目标数量设置为0.
...
void ShadowMapApp::BuildMaterials()
{
    ...
    tile0->DiffuseAlbedo = XMFLOAT4(0.8f, 0.8f, 0.8f, 1.0f);
...
void ShadowMapApp::DrawSceneToShadowMap()
{
    ...
    // 由于仅向深度缓冲区绘制, 所以把渲染目标设为空(nullptr).
    // 这样一来禁止颜色数据向渲染目标写入.
    // 也需要将PSO中的渲染目标个数(NumRenderTargets)置为0.
    ...
    // 将资源状态转换为GENERIC_READ, 以使着色器可以读取此纹理.
    mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mShadowMap->Resource(),
