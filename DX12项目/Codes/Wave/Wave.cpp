...
{
	auto waveMat = mMaterials["wave"].get();

	float& tu = waveMat->MatTransform(3, 0);  // 获取矩阵第四行第一列的值, 对应于x轴的平移
	float& tv = waveMat->MatTransform(3, 1);  // 第四行第2列对应于y轴的位置平移

	tu += 0.05f * gt.DeltaTime();
	tv += 0.05f * gt.DeltaTime();
	...
	waveMat->NumFramesDirty = gNumFrameResources;
}
...
void NormalMapApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.Lights[0].Direction = { 0.0f, -0.43589f, -0.9f };
...
void NormalMapApp::LoadTextures()
{
	std::vector<std::string> texNames = 
	{
		"bricksDiffuseMap",
		"bricksNormalMap",
		"waveDiffuseMap",  // 用以模拟波浪的第一张法线与位移贴图
		"waveNormalMap",   // 第二张法线,位移贴图
		"defaultDiffuseMap",
		"defaultNormalMap",
		"skyCubeMap"
	};
...
void NormalMapApp::BuildRootSignature()
{
	...
	texTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, 0);  // 用以绑定天空盒的立方体贴图

	CD3DX12_DESCRIPTOR_RANGE texTable1;
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 10, 1, 0);  // 用以绑定其他贴图
...
void NormalMapApp::BuildShadersAndInputLayout()
{
	...
	mShaders["waveHS"] = d3dUtil::CompileShader(L"Shaders\\wave.hlsl", nullptr, "HS", "hs_5_1");
	mShaders["waveDS"] = d3dUtil::CompileShader(L"Shaders\\wave.hlsl", nullptr, "DS", "ds_5_1");
...
void NormalMapApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData grid = geoGen.CreateGrid(30.0f, 30.0f, 256, 256);
...
void NormalMapApp::BuildPSOs()
{
	...
	wavePsoDesc.HS =
	{
		reinterpret_cast<BYTE*>(mShaders["waveHS"]->GetBufferPointer()),
		mShaders["waveHS"]->GetBufferSize()
	};
	wavePsoDesc.DS =
	{
		reinterpret_cast<BYTE*>(mShaders["waveDS"]->GetBufferPointer()),
		mShaders["waveDS"]->GetBufferSize()
	};
	wavePsoDesc.PS =
	{
	...
	wavePsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_PATCH;
...
void NormalMapApp::BuildMaterials()
{
	...
	auto wave = std::make_unique<Material>();
	wave->Name = "wave";
	wave->MatCBIndex = 2;
	wave->DiffuseSrvHeapIndex = 2;
	wave->NormalSrvHeapIndex = 3;
	wave->DiffuseAlbedo = XMFLOAT4(0.02f, 0.4f, 0.65f, 1.0f);
	wave->FresnelR0 = XMFLOAT3(0.02f, 0.02f, 0.02f);
	wave->Roughness = 0.1f;
	...
	mirror0->DiffuseAlbedo = XMFLOAT4(1.0f, 0.71f, 0.29f, 1.0f);
	mirror0->FresnelR0 = XMFLOAT3(1.0f, 0.71f, 0.29f);
...
void NormalMapApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&waveRitem->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));
	waveRitem->ObjCBIndex = 3;
	waveRitem->Mat = mMaterials["wave"].get();
	waveRitem->Geo = mGeometries["shapeGeo"].get();
	waveRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
	...
	mRitemLayer[(int)RenderLayer::Wave].push_back(waveRitem.get());
