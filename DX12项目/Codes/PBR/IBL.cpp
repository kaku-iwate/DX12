...
private:
    ...
    void BuildShadersAndInputLayout();  // 根据编译着色器时定义的define来变更漫反射模型(兰伯特或迪士尼)
	...
	void BuildBRDFLUT(ID3D12GraphicsCommandList* cmdList);
...
private:
	...
	// 每次运行时计算BRDF_LUT
	std::unique_ptr<BRDF> LUT = nullptr;
	bool GetLut = false;
...
bool NormalMapApp::Initialize()
{
	...
	LUT = std::make_unique<BRDF>(md3dDevice.Get());
...
void NormalMapApp::CreateRtvAndDsvDescriptorHeaps()
{
	...
	rtvHeapDesc.NumDescriptors = SwapChainBufferCount + 1;  // 多出来的一个rtv用来预计算IBL中环境光镜面反射部分的BRDF
...
void NormalMapApp::Draw(const GameTimer& gt)
{
	...
	// 只预计算一次BRDF, 这里的预计算可以放到初始化部分, 没必要每次循环都做一下判断
	// 但放外面还需要设置根签名绑定各种东西, 为了方便起见放到了这里
	if (!GetLut)
	{
		mCommandList->SetPipelineState(mPSOs["BRDF"].Get());
		mCommandList->RSSetViewports(1, &LUT->Viewport());
		mCommandList->RSSetScissorRects(1, &LUT->ScissorRect());

		mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(LUT->Resource(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_RENDER_TARGET));
		mCommandList->ClearRenderTargetView(LUT->Rtv(), Colors::LightSteelBlue, 0, nullptr);
		mCommandList->OMSetRenderTargets(1, &LUT->Rtv(), true, nullptr);

		BuildBRDFLUT(mCommandList.Get());
		mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(LUT->Resource(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_GENERIC_READ));
	...
	// 绑定计算好的BRDFLUT
	mCommandList->SetGraphicsRootDescriptorTable(5, LUT->Srv());
...
void NormalMapApp::LoadTextures()
{
	...
	std::vector<std::wstring> texFilenames = 
	{
		...
		L"../../Textures/snowcube1024.dds",  //desertcube1024.dds
	};
...
void NormalMapApp::BuildRootSignature()
{
	...
	CD3DX12_DESCRIPTOR_RANGE BRDFlut;
	BRDFlut.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1, 0);
	...
	slotRootParameter[5].InitAsDescriptorTable(1, &BRDFlut, D3D12_SHADER_VISIBILITY_PIXEL);
...
void NormalMapApp::BuildDescriptorHeaps()
{
	...
	int LUTindex = mSkyTexHeapIndex + 1;
	auto cpuSrv = CD3DX12_CPU_DESCRIPTOR_HANDLE(mSrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart()).Offset(LUTindex, mCbvSrvDescriptorSize);
	auto gpuSrv = CD3DX12_GPU_DESCRIPTOR_HANDLE(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart()).Offset(LUTindex, mCbvSrvDescriptorSize);
	...
	// 为BRDF_LUT创建SRV,RTV等
	LUT->BuildDescriptors(cpuSrv, gpuSrv, cpuRtv);
}
...
void NormalMapApp::BuildShadersAndInputLayout()
{
	...
	{
		//"Disney", "1",  // 取消这里的注释就可以将漫反射模型变为迪士尼的
		NULL, NULL
	};

	mShaders["PBRVS"] = d3dUtil::CompileShader(L"Shaders\\PBR.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["PBRPS"] = d3dUtil::CompileShader(L"Shaders\\PBR.hlsl", defines, "PS", "ps_5_1");
	
	mShaders["skyVS"] = d3dUtil::CompileShader(L"Shaders\\Sky.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["skyPS"] = d3dUtil::CompileShader(L"Shaders\\Sky.hlsl", nullptr, "PS", "ps_5_1");

	mShaders["BRDFVS"] = d3dUtil::CompileShader(L"Shaders\\BRDF_LUT.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["BRDFPS"] = d3dUtil::CompileShader(L"Shaders\\BRDF_LUT.hlsl", nullptr, "PS", "ps_5_1");
...
void NormalMapApp::BuildPSOs()
{
	...
	BRDFPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["BRDFVS"]->GetBufferPointer()),
		mShaders["BRDFVS"]->GetBufferSize()
	};
	BRDFPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["BRDFPS"]->GetBufferPointer()),
		mShaders["BRDFPS"]->GetBufferSize()
	};
	BRDFPsoDesc.RTVFormats[0] = DXGI_FORMAT_R16G16B16A16_FLOAT;
...
void NormalMapApp::BuildMaterials()
{
	...
	bricks0->DiffuseAlbedo = XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f);
    bricks0->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
	...
	tile0->DiffuseAlbedo = XMFLOAT4(0.5f, 0.7f, 0.5f, 1.0f);
	tile0->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
	...
	mirror0->DiffuseAlbedo = XMFLOAT4(1.0f, 0.86f, 0.57f, 1.0f);
	mirror0->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
	mirror0->Roughness = 0.1f;
	mirror0->metallic = 1.0f;

	auto mirror1 = std::make_unique<Material>();
	mirror1->Name = "mirror1";
	mirror1->MatCBIndex = 3;
	mirror1->DiffuseSrvHeapIndex = 4;
	mirror1->NormalSrvHeapIndex = 5;
	mirror1->DiffuseAlbedo = XMFLOAT4(0.3f, 0.1f, 0.5f, 1.0f);
	mirror1->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
	mirror1->Roughness = 0.3f;
	mirror1->metallic = 0.8f;

	auto mirror2 = std::make_unique<Material>();
	mirror2->Name = "mirror2";
	mirror2->MatCBIndex = 4;
	mirror2->DiffuseSrvHeapIndex = 4;
	mirror2->NormalSrvHeapIndex = 5;
	mirror2->DiffuseAlbedo = XMFLOAT4(0.8f, 0.5f, 0.5f, 1.0f);
	mirror2->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
	mirror2->Roughness = 0.8f;
	mirror2->metallic = 0.0f;
	...
	skullMat->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
	skullMat->Roughness = 0.4f;
	...
	sky->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
	sky->Roughness = 1.0f;
	
	mMaterials["bricks0"] = std::move(bricks0);
	mMaterials["tile0"] = std::move(tile0);
	mMaterials["mirror0"] = std::move(mirror0);
	mMaterials["mirror1"] = std::move(mirror1);
	mMaterials["mirror2"] = std::move(mirror2);
	...
	for (int i = 0; i < 6; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			...
			sphere->Name = "sphere" + std::to_string(i * 6 + j);
			sphere->MatCBIndex = 7 + i * 6 + j;
			sphere->DiffuseSrvHeapIndex = 4;
			sphere->NormalSrvHeapIndex = 5;
			sphere->DiffuseAlbedo = XMFLOAT4(0.9f, 0.9f, 0.9f, 1.0f);
			sphere->FresnelR0 = XMFLOAT3(0.04f, 0.04f, 0.04f);
			sphere->Roughness = 0.2 * j;
			sphere->metallic = 0.2f * i;
...
void NormalMapApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&globeRitem->World, XMMatrixScaling(2.5f, 2.5f, 2.5f)*XMMatrixTranslation(3.5f, 1.25f, 0.0f));
	...
	for(int i = 0; i < 3; ++i)
	{
		...
		XMMATRIX leftSphereWorld = XMMatrixTranslation(-7.0f, 0.5f, -5.0f + i*5.0f);
		XMMATRIX rightSphereWorld = XMMatrixTranslation(-3.5f, 0.5f, -5.0f + i*5.0f);


		XMStoreFloat4x4(&leftSphereRitem->World, leftSphereWorld);
		leftSphereRitem->TexTransform = MathHelper::Identity4x4();
		leftSphereRitem->ObjCBIndex = objCBIndex++;
		leftSphereRitem->Mat = mMaterials["mirror1"].get();
		...
		rightSphereRitem->Mat = mMaterials["mirror2"].get();
	...
	for (int i = 0; i < 6; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			std::string matName = "sphere" + std::to_string(i * 6 + j);
			auto rightSphereRitem = std::make_unique<RenderItem>();

			XMMATRIX rightSphereWorld = XMMatrixTranslation(-5.0f + j * 1.8f, 0.5f + i * 1.8f, -10.0f );

			XMStoreFloat4x4(&rightSphereRitem->World, rightSphereWorld);
			rightSphereRitem->TexTransform = MathHelper::Identity4x4();
			rightSphereRitem->ObjCBIndex = objCBIndex++;
			rightSphereRitem->Mat = mMaterials[matName].get();
			rightSphereRitem->Geo = mGeometries["shapeGeo"].get();
			rightSphereRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
			rightSphereRitem->IndexCount = rightSphereRitem->Geo->DrawArgs["sphere"].IndexCount;
			rightSphereRitem->StartIndexLocation = rightSphereRitem->Geo->DrawArgs["sphere"].StartIndexLocation;
...
void NormalMapApp::BuildBRDFLUT(ID3D12GraphicsCommandList* cmdList)
{
